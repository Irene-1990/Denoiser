...
template<typename T>
inline T get_ptree_item(ptree &pt, const std::string &name, const std::string &desc, const T &value) {
  pt.put(name+".desc", desc);
  if (!zjucad::has(name+".value", pt))
    pt.put(name+".value", value);
  return pt.get<T>(name+".value");
}
